...
export function SurahPageClient({ surahId }: { surahId: number }) {
  ...
  useEffect(() => {
    if (surahId) setSelectedSurah(surahId);
    ...
    <main className="flex flex-col h-screen bg-background selection:bg-primary/30">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className={cn(
          ...
          <div className={cn(
            "absolute inset-y-0 left-0 w-80 bg-background shadow-2xl transition-transform duration-300",
            isSurahSidebarOpen ? "translate-x-0" : "-translate-x-full"
          )}>
            <SurahSidebar isMobile onSelect={() => setIsSurahSidebarOpen(false)} onClose={() => setIsSurahSidebarOpen(false)} />
          </div>
        </div>
        ...
        <div className={cn(
          ...
          <div className={cn(
            "absolute inset-y-0 right-0 w-80 bg-background shadow-2xl transition-transform duration-300",
