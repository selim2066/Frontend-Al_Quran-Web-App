...
// !v2
"use client";
import { searchAyahs, fetchSurahs } from "../../surah/services/quranApi";
import {
  CommandDialog,
  CommandGroup,
  CommandInput,
...
export function SearchDialog({ trigger }: SearchDialogProps) {
  ...
  // Surah list for when query is short
  const { data: surahs } = useQuery({
    queryKey: ["surahs"],
    queryFn: fetchSurahs,
  });

  // Backend search for ayahs
  const { data: searchResults, isLoading: isSearching } = useQuery({
    queryKey: ["search", query],
    queryFn: async () => {
      if (query.length < 3) return null;
              ...
              {searchResults?.map((result: any) => (
                ...
                >
                  ...
                  <div className="flex flex-col">
                    <span
                      className="text-sm line-clamp-1"
                      dangerouslySetInnerHTML={{ __html: result.translation_text }}
                    />
                    <span className="text-[10px] text-muted-foreground">
                      {result.verse_key}
                    </span>
